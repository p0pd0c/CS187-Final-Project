...
deathVaccineRatesPlot <- ggplot(cov19Average, aes(x = cov192$`mean(vaccineRate, na.rm = TRUE)`, y = cov192$`mean(deathRate, na.rm = TRUE)`))+
  geom_line()+
  labs(x = "Vaccine Rate",
       y = "Death Rate",
       title = "Death Rate and Vaccine Rate")

deathInfectionRatesPlot <- ggplot(cov19Average, aes(x = cov192$`mean(infectionRate, na.rm = TRUE)`, y = cov192$`mean(deathRate, na.rm = TRUE)`))+
  geom_line()+
...
plot_usmap(data = cov19StatesOnly,
           ...
           regions = 'state',
           color = "black")+
  scale_fill_continuous(low = 'light blue', high = 'red')+
